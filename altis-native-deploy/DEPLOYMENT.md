...
For a single-domain deployment, use `VITE_POCKETBASE_URL=/hcgi/platform` and configure the Node app to start PocketBase locally with these environment variables:

- `POCKETBASE_BINARY=/path/to/linux/pocketbase`
- `POCKETBASE_DATA_DIR=/path/to/persistent/data`
- `POCKETBASE_PORT=8090`
- `PB_ENCRYPTION_KEY`
- `PB_SUPERUSER_EMAIL`
- `PB_SUPERUSER_PASSWORD`

The Node entry server proxies `/hcgi/platform/*` to PocketBase, so admin login and CMS requests stay on `altistravels.com` without a subdomain.
