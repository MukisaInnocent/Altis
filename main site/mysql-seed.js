...
await ensureUser(adminEmail, bcrypt.hashSync(adminPassword, 10));
...
await upsertSiteContent('contact', {
...
async function seedTable(tableName, rows) {
  if (await countRows(tableName)) return;
  for (const row of rows) await insertRow(tableName, row);
}

await seedTable('destinations', [
...
await seedTable('packages', [
  { title: 'Gorilla Trekking Expedition', destination: 'Bwindi Impenetrable Forest, Uganda', days: 4, price: 6850000, image: 'https://images.hostinger.com/c8d9efe2-440c-4c0d-b269-16fd05edb138.png', description: 'Four days through the highlands: gorilla permit, lodge stay, Batwa community walk and Lake Bunyonyi wind-down.', featured: 1 },
  { title: 'Murchison Falls Safari', destination: 'Murchison Falls NP, Uganda', days: 3, price: 2450000, image: 'https://images.hostinger.com/110ca975-1703-4f2e-8e46-ae84dd12453a.png', description: 'Game drives, a Nile boat cruise to the base of the falls, and a hike to the top.', featured: 1 },
  { title: 'Dubai City Escape', destination: 'Dubai, UAE', days: 5, price: 5300000, image: 'https://images.hostinger.com/f6da86f0-b16c-43af-a1e8-976f2ad4df16.png', description: 'Flights from Entebbe, 4-star hotel, desert safari, Burj Khalifa entry and UAE visa processing.', featured: 1 },
  { title: 'Zanzibar Beach Retreat', destination: 'Zanzibar, Tanzania', days: 6, price: 3950000, image: 'https://images.hostinger.com/7c37ccfb-ca26-4447-9cff-8d7ac49eb1a0.png', description: 'Beachfront resort, Stone Town walking tour, spice farm visit and sunset dhow cruise.', featured: 1 }
]);
...
console.log(`Seeded MySQL database for Altis Voyage. Admin login: ${adminEmail}`);
